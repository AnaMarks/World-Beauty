...
export default class CadastroCliente extends Cadastro {
    ...
    public cadastrar(): Cliente {
        ...
        let ddd = partesTell[0];
        let numero = partesTell[1];
        let telefone = new Telefone(ddd, numero);
        cliente.getTelefones.push(telefone);

        novo = this.entrada.receberTexto(`Digitar outro Telefone? Sim / Não ? `);
        while (novo == 'Sim' || novo == 'sim') {
        tell = this.entrada.receberTexto(`Por favor digite o numero do telefone no padrão DDD NUMERO:`);
        partesTell = tell.split(' ');
        ddd = partesTell[0];
        numero = partesTell[1];
